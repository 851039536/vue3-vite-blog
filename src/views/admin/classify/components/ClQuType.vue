...
import { quVisible, quVisible2, resClassifyType, classifyTypeState, state, resClassify } from '../data'
...
const GetFy = async (identity: number, type: string | number, pageindex: number, pagesize: number) => {
  classifyType.GetFy(identity, type, pageindex, pagesize).then((res) => {
  ...
  await classifyType.Add(classifyTypeState).then(() => {
    message.loading('添加中..', 1.5).then(
      async () => {
        ...
        GetFy(3, classifyTypeState.classifyId, 1, 100)
