...
<template>
  ...
  <section class="fade-in">
    <div class="indexcolumn">
      <div class="border-b flex border-gray-100 py-5 px-6 items-center justify-between">
        <span>文档列表</span>
        <a-select
          ref="select"
          :bordered="false"
      ...
      <div class="indexcolumn-list" style="max-height: 22rem">
        <ul>
          <li v-for="item in state.resultData" :key="item.id">
            <div class="mr-1">
            ...
            <span
              ><a class="text-lg text-gray-700" @click="method.skip(item.id)">{{ item.title }}</a>
            </span>
            <span class="ml-auto text-base">{{ item.classify.name }}</span>
          </li>
        </ul>
      </div>
    </div>
...
//@import "./index.scss";
.indexcolumn {
  @apply bg-white rounded m-auto shadow w-[100%];
  .indexcolumn-list {
